...
import {pbkdf2Encode} from './index';
...
describe('pbkdf2Encode', (): void => {
  ...
  for (const onlyJs of [false, true]) {
    describe(`onlyJs=${(onlyJs && 'true') || 'false'}`, (): void => {
      it('creates known iterations', (): void => {
        expect(u8aToHex(pbkdf2Encode(TEST_PASSWORD, KNOWN_SALT, 2048, onlyJs).password)).toEqual(
  ...
  perfWasm('pbkdf2Encode', 8, (input, onlyJs) => pbkdf2Encode(input, input, undefined, onlyJs));
});
