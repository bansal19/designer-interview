      ...
      for (const onlyJs of [false, true]) {
        describe(`onlyJs=${(onlyJs && 'true') || 'false'}`, (): void => {
          it('returns an hex representation (string)', (): void => {
            expect(keccakAsU8a(input, bitLength, onlyJs)).toEqual(output[bitLength]);
          });

          it('returns an hex representation (Buffer)', (): void => {
            expect(keccakAsU8a(Buffer.from(input), bitLength, onlyJs)).toEqual(output[bitLength]);
          });

          it('returns an hex representation (Uint8Array)', (): void => {
            expect(keccakAsU8a(stringToU8a(input), bitLength, onlyJs)).toEqual(output[bitLength]);
          });
        });
      }
