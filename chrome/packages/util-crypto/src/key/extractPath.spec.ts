...
import type {ExtractResult} from './extractPath';
...
describe('keyExtractPath', (): void => {
  ...
  it('does not extract from invalid paths (1)', (): void => {
    expect((): ExtractResult => keyExtractPath('1/2')).toThrow(/does not match input/);
  });

  it('does not extract from invalid paths (2)', (): void => {
    expect((): ExtractResult => keyExtractPath('hello')).toThrow(/does not match input/);
  });
});
