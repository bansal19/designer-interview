  ...
  for (const onlyJs of [false, true]) {
    describe(`onlyJs=${(onlyJs && 'true') || 'false'}`, (): void => {
      it('returns a 64-bit value by default', (): void => {
        expect(xxhashAsHex('abc', undefined, onlyJs)).toEqual('0x990977adf52cbc44');
      });

      it('returns a 128-bit value (as specified)', (): void => {
        expect(xxhashAsHex('abc', 128, onlyJs)).toEqual('0x990977adf52cbc440889329981caa9be');
      });

      it('returns a 256-bit value (as specified)', (): void => {
        expect(xxhashAsHex('abc', 256, onlyJs)).toEqual(
