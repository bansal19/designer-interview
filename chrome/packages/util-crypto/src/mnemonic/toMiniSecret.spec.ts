...
describe('mnemonicToMiniSecret', (): void => {
  ...
  it('creates a known minisecret from a non-english mnemonic', (): void => {
    const mnemonic =
      '엉덩이 능동적 숫자 팩시밀리 비난 서적 파출소 도움 독창적 인생 상류 먼지 답변 음반 수박 사업 노란색 공사 우체국 특급 도대체 금지 굉장히 고무신';

    expect(() => mnemonicToMiniSecret(mnemonic, 'testing')).toThrow();
    expect(u8aToHex(mnemonicToMiniSecret(mnemonic, 'testing', koreanWords))).toEqual(
  ...
  for (const onlyJs of [false, true]) {
    describe(`onlyJs=${(onlyJs && 'true') || 'false'}`, (): void => {
      it('generates a valid seed', (): void => {
        expect(u8aToHex(mnemonicToMiniSecret(MNEMONIC, undefined, undefined, onlyJs))).toEqual(
          SEED,
        );
      });

      it('fails with non-mnemonics', (): void => {
        expect(() => mnemonicToMiniSecret('foo bar baz', undefined, undefined, onlyJs)).toThrow(
          /mnemonic specified/,
        );
      });
      ...
      tests.forEach(([mnemonic, , seed], index): void => {
        it(`Created correct seed for ${index}`, (): void => {
          expect(u8aToHex(mnemonicToMiniSecret(mnemonic, 'Substrate', undefined, onlyJs))).toEqual(
