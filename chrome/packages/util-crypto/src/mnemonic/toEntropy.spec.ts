...
import {french as frenchWords} from './wordlists/index';
...
describe('mnemonicToEntropy', (): void => {
  ...
  it('has the correct entropy for non-Englist mnemonics', (): void => {
    const mnemonic =
      'pompier circuler pulpe injure aspect abyssal nuque boueux équerre balisage pieuvre médecin petit suffixe soleil cumuler monstre arlequin liasse pixel garrigue noble buisson scandale';

    expect(() => mnemonicToEntropy(mnemonic)).toThrow();
    expect(mnemonicToEntropy(mnemonic, frenchWords)).toEqual(
      new Uint8Array([
        189, 230, 55, 17, 65, 33, 40, 4, 106, 9, 11, 88, 227, 26, 229, 76, 59, 123, 200, 55, 177,
        232, 158, 66, 34, 54, 93, 54, 255, 74, 137, 70,
      ]),
    );
  });
});
