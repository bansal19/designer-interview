...
export function scryptFromU8a(data: Uint8Array): Result {
  ...
  // FIXME We have to loosen the protection level here to facilitate JSON import
  // JSON is using a different N param and it will throw an error without this change
  if (p !== DEFAULT_PARAMS.p || r !== DEFAULT_PARAMS.r) {
    throw new Error('Invalid injected scrypt params found');
  }
