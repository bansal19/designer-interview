    ...
    describe(`onlyJs=${(onlyJs && 'true') || 'false'}`, (): void => {
      for (const {input, output} of TESTS) {
        it(`creates known sha-256 hash, ${output}`, (): void => {
          expect(sha256AsU8a(hexToU8a(input), onlyJs)).toEqual(hexToU8a(output));
        });
      }
    });
  }

  perfWasm('sha256AsU8a', 128000, (input, onlyJs) => sha256AsU8a(input, onlyJs));
});
